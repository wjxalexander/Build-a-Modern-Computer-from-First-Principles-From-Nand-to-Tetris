...
function keywordAndSymbolTokenGenerate(keyword, symbol) {
    ...
    return { xml: "<" + `${symbol}> ${keyword} </${symbol}>`, json: { content: keyword, tag: symbol } }
}

function integerTokenGenerater(number) {
    return { xml: `<integerConstant> ${number} </integerConstant>`, json: { content: number, tag: "integerConstant" } }
}

function stringTokenGenerater(string) {
    const stringCotent = string.substring(1, string.length - 1)
    return { xml: `<stringConstant> ${stringCotent} </stringConstant>`, json: { content: stringCotent, tag: "stringConstant" } }
}
function identifierTokenGenrater(identifier) {
    return { xml: `<identifier> ${identifier} </identifier>`, json: { content: identifier, tag: "identifier" } }
